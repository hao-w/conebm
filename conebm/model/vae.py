...
import numpy as np
from sebm.net import Reshape, cnn_block, deconv_block, mlp_block, cnn_output_shape
...
class SUPEncoder(nn.Module):
    def __init__(self, num_classes, im_height, im_width, input_channels, channels, kernels, strides, paddings, hidden_dims, latent_dim, activation='ReLU', reparameterized=True, **kwargs):
        super().__init__()
        ...
        cnn_output_dim = out_h * out_w * channels[-1]
        self.mlp_net = mlp_block(cnn_output_dim, hidden_dims, activation, **kwargs)
        self.mean = nn.Linear(hidden_dims[-1]+num_classes, latent_dim)
        self.log_std = nn.Linear(hidden_dims[-1]+num_classes, latent_dim)
        self.reparameterized = reparameterized
        
    def forward(self, x, y):
        mean, std = self.latent_params(x, y)
        q = dists.Normal(mean, std)
        z = q.rsample() if self.reparameterized else q.sample()
        log_qz = q.log_prob(z).sum(-1)
        return z, log_qz
...
class SUPDecoder(nn.Module):
    def __init__(self, device, num_classes, im_height, im_width, input_channels, channels, kernels, strides, paddings, hidden_dims, latent_dim, activation, dec_paddings, **kwargs):
        super().__init__()
        self.device = device
        self.prior_mean = torch.zeros(latent_dim, device=self.device)
        self.prior_log_std = torch.zeros(latent_dim, device=self.device)
        ...
        hidden_dims.reverse()
        hidden_dims.append(mlp_output_dim)
        self.mlp = mlp_block(latent_dim+num_classes, hidden_dims, activation, **kwargs)
        self.reshape = Reshape([deconv_input_channels, deconv_input_h, deconv_input_w])
        self.deconv_net = deconv_block(deconv_input_h, deconv_input_w, deconv_input_channels, channels, kernels, strides, dec_paddings, activation, last_act=False, batchnorm=False, **kwargs)
        self.deconv_net = nn.Sequential(*(list(self.deconv_net) + [nn.Sigmoid()]))    
    ...
    def forward(self, z, x, y):
        S, B, C, H ,W = x.shape
        h = self.reshape(self.mlp(toch.cat(z.view(S*B, -1), y), 0))
        recon = self.deconv_net(h).view(S, B, C, H, W)
        log_pz = dists.Normal(self.prior_mean, self.prior_log_std.exp()).log_prob(z).sum(-1)
        ll = - self.binary_cross_entropy(recon, x)
        return recon, ll, log_pz